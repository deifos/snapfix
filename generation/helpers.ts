...
export async function onGenerationComplete(response: GenerationResponse):Promise<Record<string,string>> {
    ...
    return new Promise<Record<string,string>>(async (res, rej) =>  {
        try {
            let result: string = "";
            for (const artifact of response.imageArtifacts) {
                result = artifact.getBinary_asB64();
            }
            res({ result });
        } catch (error) {
            rej("SAI: Error converting");
        }
    });
}